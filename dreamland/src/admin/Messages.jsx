...
import { getPaginatedMessagesFromDatabase, editMessageFromDatabase, deleteMessageFromDatabase } from '../actions/messageAction';
  ...
  useEffect(() => {
    ...
    setDatasSearch(
      messages.filter(data => 
          [data.content, data.email, data.fullname, data.phone]
    ...
    <div className="p-8">
      <div className="min-h-[80vh] max-w-6xl mx-auto">
        ...
        { datasSearch.length>0 ? (
          <MessageList get={get} filteredMessages={datasSearch} />
        ):(
          <>
          {loading ? (
        ...
        { messages.length > pageSize && datasSearch.length == 0 ? (
